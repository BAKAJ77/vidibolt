...
namespace Volt
{
	...
	void Connection::OnTransmissionCompletion(const system::error_code& ec, size_t bytesSent, system::error_code& ecOut)
	{
			...
			if (!errorCodeRecieve)
			{
				...
				asio::async_read(this->socket, asio::buffer(recieveBuffer.data() + sizeof(Header), payloadSizeBytes),
					asio::transfer_exactly(payloadSizeBytes), boost::bind(&Connection::OnRecieveCompletion,
						...
						std::ref(recieveBuffer), std::ref(errorCodeRecieve)));
			}
		}
